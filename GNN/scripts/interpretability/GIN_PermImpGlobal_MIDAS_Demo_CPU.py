...
"""
GIN_PermImpGlobal_MIDAS_Demo_CPU
========================================

Feature permutation importance for a trained GIN model.

This script evaluates how much a trained node-classification GIN depends on the
feature permutation and measuring performance changes on the same
...
- For each fold:
   ...
   - For each permutation seed:
      – Compute column-wise feature permutation importance for a fold model. 
...
- GINBiDir_KNN{KNN}_foldID{fold}_CVTrainSet_permScores_{seedspan}.csv  
- GINBiDir_KNN{KNN}_foldID{fold}_CVTestSet_permScores_{seedspan}.csv  
- GINBiDir_KNN{KNN}_foldID{fold}_HeldOut_permScores_{seedspan}.csv
...
class Config:
    ...
    seeds: str = "0:499"                       # range "start:end" (inclusive) or comma-list "0,1,2"
...
def calc_perm_imp(
   ...
   train_idx: np.ndarray,
    ...
    """
    Column-wise feature permutation importance for a fold model.

    For each feature column and each seed, shuffle that column's values across
    all nodes (edges fixed), recompute predictions on the fold test set
    and the global held-out set, and record AUC deltas vs baseline.

    """
    
    # --- Per-worker logging bootstrap ---
    try:
        logs_dir = (log_root or (Path.cwd() / "logs" / "perm")).resolve()
        ...
        log_file = logs_dir / f"perm_fold{fold_id}.log"
        setup_logging(verbosity="INFO", log_file=log_file)

        logging.info("Worker logging ready | fold=%s | pid=%s", fold_id, os.getpid())
    except Exception as e:
        print(f"[calc_perm_imp] logging init failed for fold={fold_id}: {e}", file=sys.stderr)
    ...
    model = GNN(
        n_features=n_features, n_classes=n_classes, n_hidden=int(n_hidden),
        num_layers=int(num_layers), num_layers_pre=int(num_layers_pre),
        num_layers_post=int(num_layers_post), dropout=float(dropout),
        rate=float(rate), eps=float(eps), train_eps=bool(train_eps),
    ).to(device)
    ...
    if out_dir is not None:
        ...
        out_dir = DEFAULT_OUTPUT_DIR / "permFeatImp"
    ...
    for feat_i, feat in enumerate(feature_names, start=1):
        
        logging.info(f"[fold {fold_id}] Feature {feat_i}/{len(feature_names)}: '{feat}'")
        ...
        for seed in seeds:
            ...
            X_perm.loc[:, feat] = X_perm.loc[:, feat].to_numpy()[perm_idx]
            
            
            # Create Data object
            
            Xgraph = torch.tensor( X_perm.values, dtype=torch.float)
            ygraph = torch.tensor(yy.values, dtype=torch.float)
            
            
            data = Data(x=Xgraph, edge_index=Network, y=ygraph)

            
            data.train_mask = torch.as_tensor(idx_all.isin(pd.Index(kept_train_ids)), dtype=torch.bool)
            data.test_mask  = torch.as_tensor(idx_all.isin(pd.Index(X_val.index)), dtype=torch.bool)
            data.val_mask   = torch.as_tensor(idx_all.isin(pd.Index(nodes_heldout)), dtype=torch.bool)
            ...
            df_ht = get_predictions(
                ...
                mask_name="val_mask",
                fold_id=fold_id,
                set_label="Held-out (feat shuffle)",
                node_names=xx.index,
            )
            ...
            rows_train.append({
                "CV_fold": fold_id, "feature": feat, "permutation": int(seed),
    ...
    pd.DataFrame(rows_train).to_csv(
        out_dir / f"GINBiDir_KNN{neighbors[0]}_foldID{fold_id}_CVTrainSet_permScores_{seed_span}.csv",
        index=False
    )
    pd.DataFrame(rows_test).to_csv(
        out_dir / f"GINBiDir_KNN{neighbors[0]}_foldID{fold_id}_CVTestSet_permScores_{seed_span}.csv",
        index=False
    )
    pd.DataFrame(rows_ht).to_csv(
        out_dir / f"GINBiDir_KNN{neighbors[0]}_foldID{fold_id}_HeldOut_permScores_{seed_span}.csv",
        index=False
    )
...
def parse_args() -> Config:
    ...
    parser.add_argument("--seeds", type=str, default="0:499",
...
def main(cfg: Config) -> None:
    ...
    logs_dir = (cfg.output_dir / "logs" / "perm").resolve()
    logs_dir.mkdir(parents=True, exist_ok=True)
    ...
    fold_args = [
        dict(
            ...
            seeds=seeds_list,
            out_dir=cfg.output_dir / "permFeatImp",
            log_root=logs_dir,
        )
    ...
    results = Parallel(n_jobs=-1, backend="loky")(delayed(calc_perm_imp)(**kwargs) for kwargs in fold_args)
